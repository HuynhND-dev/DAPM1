...
@section('content')
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Danh sách thành viên đã vô hiệu</h4>
                    <p class="card-description">
                        Các thành viên trong danh sách này sẽ bị xoá vĩnh viễn sau 60 ngày!
                        <a href="{{route('create-user')}}" class="btn btn-outline-primary m-3 float-right">Thêm mới</a>
{{--                        <a href="{{ url()->previous() }}" class="btn btn-outline-success m-3 float-right">Thành viên đang hoạt động</a>--}}
                    </p>
                            ...
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="/admin/user/edit/{{$user->user_ID}}" class="btn btn-outline-primary btn-sm">Chỉnh sửa</a>

                                        <button type="button" class="btn btn-outline-warning btn-sm" data-bs-toggle="modal" data-bs-target="#Modal{{$user->user_ID}}">
                                            Khôi phục
                                        </button>

                                        <div class="modal fade" id="Modal{{$user->user_ID}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        Bạn có chắc muốn khôi phục tài khoản {{$user->username}}?
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal">Huỷ</button>
                                                        <a href="/admin/user/enable-user/{{$user->user_ID}}" class="btn btn-success">Xác nhận</a>
                                                    </div>
                                                    </form>
                                                </div>
                                            </div>
